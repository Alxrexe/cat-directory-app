    ...
    >
      <Link
        ...
        className={cn(
          ...
          "porcelain flex h-14 items-center gap-3 rounded-full py-1.5 pr-5 pl-1.5 transition-transform duration-200 ease-[var(--ease-cozy)] hover:-translate-y-0.5 active:translate-y-px max-sm:h-12 max-sm:p-1.5",
        )}
      >
        <CatMark className="size-11 shrink-0 max-sm:size-9" />
        <span className="flex flex-col gap-1.5 max-sm:sr-only">
          <span className="font-display text-[1.15rem] leading-none tracking-[-0.01em] text-slate max-sm:text-base">
            <span className="font-bold">Michi</span>
      ...
      <div className={cn(!shownAtMount && "opacity-0", "flex items-center gap-2 sm:gap-3")}>
        <div className="porcelain flex h-12 items-center rounded-full px-1.5 max-sm:h-11">
